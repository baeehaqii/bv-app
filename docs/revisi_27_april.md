...
**To-Do:**
- [ ] Pindahkan `Step::make('Brief')` ke posisi setelah `Step::make('Campaign Information')` dan sebelum `Step::make('Select KOL')`
...
**To-Do:**
- [ ] Hapus opsi `'auto' => 'Auto (Based on Budget Range)'` dari `ToggleButtons margin_type`
- [ ] Set default `margin_type` ke `'custom'`
- [ ] Ubah kolom `margin_type` di migration agar default-nya `'custom'`
- [ ] Sesuaikan logika kalkulasi di `InternalBudgetForm.php` yang merujuk ke `margin_type === 'auto'`
...
**To-Do:**
- [ ] Tambah tombol **Approve** dan **Reject** di halaman `EditInternalBudget.php`
- [ ] Saat approve: update status internal budget → `approved`, trigger `tryActivateCampaign()`
- [ ] Saat reject: update status → `rejected`, tambah field `rejection_notes`
- [ ] Auto-generate data External Budget dari Internal Media Plan saat status berubah ke "To Client"
...
**To-Do:**
- [ ] Sembunyikan kolom/field `followers` di resource Campaign Ongoing (comment out, bukan hapus)
...
**To-Do:**
- [ ] Review logika `importKolsFromCsv()` — pastikan `row_number` increment sesuai urutan baris CSV
- [ ] Tambah sorting eksplisit setelah import jika diperlukan
- [ ] Test dengan file CSV untuk memastikan urutan benar
