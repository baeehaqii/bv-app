...
<div
    ...
    <div class="flowforge-column-header flex items-center justify-between py-2.5 px-3">
        <div class="flex items-center gap-1.5">
            ...
            @if($isSemantic)
                ...
                <x-filament::badge tag="div" :color="$resolvedColor" class="ms-2">
    ...
    <!-- Column Content -->
    <div data-column-id="{{ $columnId }}" @if($this->getBoard()->getPositionIdentifierAttribute()) x-sortable
    x-sortable-group="cards" @end.stop="handleSortableEnd($event)" @endif @if(isset($column['total']) && $column['total'] > count($column['items']))
        @scroll.throttle.100ms="handleColumnScroll($event, '{{ $columnId }}')" @endif
        class="flowforge-column-content p-2 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain kanban-cards"
        style="max-height: calc(100vh - 13rem);">
        @if (isset($column['items']) && count($column['items']) > 0)
            @foreach ($column['items'] as $record)
                <x-flowforge::card :record="$record" :config="$config" :columnId="$columnId"
            ...
            <div class="py-3 text-center">
                @if(isset($column['total']) && $column['total'] > count($column['items']))
                    ...
                    <div x-intersect.margin.300px="handleSmoothScroll('{{ $columnId }}')" class="w-full">
                        ...
                        <div x-show="isLoadingColumn('{{ $columnId }}')" x-transition
