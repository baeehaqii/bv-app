...
class GenerateQuotationAction
{
    ...
    /**
     * Create a table action version for use in tables
     */
    public static function tableAction(): \Filament\Tables\Actions\Action
    {
        return \Filament\Tables\Actions\Action::make('generateQuotation')
            ->label('Quotation')
            ->icon('heroicon-o-document-arrow-down')
            ->color('success')
            ->tooltip('Generate Quotation PDF')
            ->action(function (MediaPlan $record) {
                // Check if there are selected KOLs
                $selectedKols = $record->selectedKols()->get();
